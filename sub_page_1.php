...
<body id="body">
    ...
    <div class="wrap">
        ...
        <section class="container">
            <div class="panels panels_sub panels_search_result clear" id="panels_sub">
                        ...
                        <div class="panel_content_area">
                            <div class="panel_content">
                                <div class="panel_content_text mainDesc clear">
                                    ...
                                    <div class="date_box date_box_all active">
                                        All
                                    </div>
                                    <?php
                                    $con=mysqli_connect("localhost", "tmxlraos","","tmxlraos");
                                    $sql="select * from eflux_table_1  order by num desc limit 4";
                                    $result=mysqli_query($con, $sql);
                                    while($row=mysqli_fetch_array($result)){
                                        $month=$row["month"];
                                ?>
                                    <div class="date_box date_box_month"
                                        onclick=" get_page('<?=substr($month,0,1);?>')">
                                        <?=substr($month,1,6);?>
                                    </div>
                                    <?php
                                    }
                ...
                <?php
                                    $con=mysqli_connect("localhost", "tmxlraos","","tmxlraos");
                                    $sql="select * from eflux_table_1  order by num desc";
                                    $result=mysqli_query($con, $sql);
                                    while($row=mysqli_fetch_array($result)){
                                        $num=$row["num"];
                                        $tag=$row["tag"];
                ...
                <?php
                                ...
                                ?>
                <div id="container_search_ajax" class="container_search_result">
    ...
    $(document).ready(function() {
        var mql_2 = window.matchMedia("screen and (min-width: 700px)");
        if (mql_2.matches) {
            $(".panels")
                .on("resize", function() {
                    var width = 0;
